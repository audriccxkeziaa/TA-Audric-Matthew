...
export default function ProductPicker({ open, onClose, onSelect }) {
  ...
  // Enter dari barcode scanner → auto-select jika tepat 1 hasil
  function handleKeyDown(e) {
    if (e.key !== "Enter") return;
    e.preventDefault();
    if (results.length === 1 && !isFetching) {
      onSelect(results[0]);
      handleClose();
    }
  }
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Pilih Produk dari Katalog">
      <input
        ref={inputRef}
        autoFocus
        placeholder="Ketik nama / kode, atau scan barcode lalu Enter…"
        value={q}
      ...
      <p className="mt-1 text-[11px] text-slate-400">
        Tip: scan barcode barang dengan alat scanner → tekan Enter → otomatis terpilih jika 1 hasil.
      </p>
